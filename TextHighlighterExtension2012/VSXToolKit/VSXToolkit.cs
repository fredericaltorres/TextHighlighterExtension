...
namespace VSXToolkit {
    ...
    public class VSPackage : Package {
        ...
        /// <summary>
        /// Replace selected text http://msdn.microsoft.com/en-us/library/envdte.editpoint.replacetext(v=vs.80).aspx
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// http://msdn.microsoft.com/en-us/library/vstudio/y1cksdak.aspx
        /// </summary>
        /// <param name="text"></param>
        public void SetCurrentTextFile(string text){
            ...
            TextDocument doc = (TextDocument)(dte.ActiveDocument.Object("TextDocument"));
            var p            = doc.StartPoint.CreateEditPoint();
            var movePoint    = doc.EndPoint.CreateEditPoint();
            p.ReplaceText(movePoint, text, (int)vsEPReplaceTextOptions.vsEPReplaceTextKeepMarkers);
        }
        ...
        public void MsgBoxError(string m){

            // Show a Message Box to prove we were here            
            Guid clsid = Guid.Empty;
            int result;
            uiShell.ShowMessageBox( 0, ref clsid, "TextHighlighter Extension", m, string.Empty, 0, OLEMSGBUTTON.OLEMSGBUTTON_OK, OLEMSGDEFBUTTON.OLEMSGDEFBUTTON_FIRST, OLEMSGICON.OLEMSGICON_CRITICAL, 0, out result);
        }
        ...
        public void MsgBox(string m){
            ...
            Guid clsid = Guid.Empty;
            int result;
            uiShell.ShowMessageBox( 0, ref clsid, "TextHighlighter Extension", m, string.Empty, 0, OLEMSGBUTTON.OLEMSGBUTTON_OK, OLEMSGDEFBUTTON.OLEMSGDEFBUTTON_FIRST, OLEMSGICON.OLEMSGICON_INFO, 0, out result);
        }
        /// <summary>
        /// 
        /// </summary>
       ...
       public string CloseCurrentDocument() {
        ...
        public void InformUser(string m, string outputWindow){

            if(m!=null){
            this.GetCustomOutputWindow(outputWindow).OutputString(m+"\n");
            }
            this.GetCustomOutputWindow(outputWindow).Activate();
        }
        ...
        public void CreateNewTextFile(string newFileName, string text) {

            this.dte.ItemOperations.NewFile("General\\Text File", newFileName);
            EnvDTE.TextDocument doc;
            EnvDTE.EditPoint movePoint, editPoint;
            doc = this.dte.ActiveDocument.Object("TextDocument") as EnvDTE.TextDocument;
            movePoint = doc.EndPoint.CreateEditPoint();
            editPoint = doc.StartPoint.CreateEditPoint();
            editPoint.Insert(text);

            editPoint.StartOfDocument();
            movePoint.EndOfDocument();
        }
    }
}
